...
richness <- estimate_richness(psd4)
head(richness)

w1 = wilcox.test(richness$Observed ~ sample_data(psd4)$groups)
w1

w2 = wilcox.test(richness$Chao1 ~ sample_data(psd4)$groups)
w2

w3 = wilcox.test(richness$Shannon ~ sample_data(psd4)$groups)
w3

w4 = wilcox.test(richness$Simpson ~ sample_data(psd4)$groups)
w4
```
...
richness1 <- estimate_richness(psd4_mdd)
head(richness1)

x1 = wilcox.test(richness1$Observed ~ sample_data(psd4_mdd)$groups)
x1

x2 = wilcox.test(richness1$Chao1 ~ sample_data(psd4_mdd)$groups)
x2

x3 = wilcox.test(richness1$Shannon ~ sample_data(psd4_mdd)$groups)
x3

x4 = wilcox.test(richness1$Simpson ~ sample_data(psd4_mdd)$groups)
x4
```
### Diversidad beta 
...
```{r}
ps.pr<- transform_sample_counts(psd4, function(otu) otu/sum(otu))
ordin<- ordinate(ps.pr, method="NMDS", distance="bray")
plot_ordination(ps.pr, ord, color="groups")
```
...
```{r}
dist = phyloseq::distance(ps.pr, method="bray", binary = TRUE)

metada<- data.frame(sample_data(ps.pr))
anosim(dist, metadata$groups)
...
ps.rel = transform_sample_counts(psd4, function(x) x/sum(x)*100)
...
ggplot(ps.melt_sum, aes(x = Sample, y = Abundance, fill = Family)) + 
  ...
  theme(legend.position = "right", 
        ...
        axis.text.x.bottom = element_text(angle = -90))
ggsave("Plots/abundancia_familia_grupos.png", dpi=600,width = 21,
  height = 13)
```
...
ggplot(ps.melt_sum, aes(x = Sample, y = Abundance, fill = Family)) + 
  ...
  theme(legend.position = "right", 
        strip.background = element_blank(), 
        axis.text.x.bottom = element_text(angle = -90))
ggsave("Plots/abundancia_familia_mdd.png", dpi=600,width = 21,
  height = 13)
```
