...
fantaxtic_bar(top, color_by = "Family", label_by = "Genus", facet_by = "groups", grid_by = NULL, other_color = "Grey") -> ptop15.2
```

```{r}
#Para genero
top <- names(sort(taxa_sums(psd5), decreasing=TRUE))[1:222]
ps.top <- transform_sample_counts(psd5, function(OTU) OTU/sum(OTU))
ps.top <- prune_taxa(top, ps.top)
plot_bar(ps.top, x="weeks", fill="Genus") + facet_wrap(~groups, scales="free_x")
